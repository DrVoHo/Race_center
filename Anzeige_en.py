...
def Quali():  #Statemachine = 20
	...
	if g.Quali_Zeit >= g.timer_time:
		...
		for i in range(0,int(g.Anzahl_Spuren/2)): #max 2 display
			...
			Message[4*i] ="  " + temp[4*i] +"  Laps      " + temp[4*i+2]  
			Message[4*i+1] =temp[4*i+1] +"  Time    " + temp[4*i+3] 
...
def Startaufstellung():	
	...
	Message[0] ="    Place Cars      " 
	Message[1] =" in Startposition   "  
	Message[2] =" '*' Cancel         "
	Message[3] =" '#' Continue       "
	for i in range(0,4):
		Message[i+4] = "                    "
...
def Rundenzeit():
	...
	for i in range(0,int(g.Anzahl_Spuren/2)): #max 2 display
		...
		Message[4*i+2] =temp[4*i] +"  Last L  " + temp[4*i+2]  
		Message[4*i+1+2] =temp[4*i+1] +"  Best L  " + temp[4*i+3] 
...
def Einstellungen():
	...
	Message[0] ="A: Time Quali  %0.2umin"%(int(g.Quali_Zeit/60))
	Message[1] ="B: Laps       %0.3u   "%(g.Rennen_Runden)
	Message[2] ="C: Advantage        "
	Message[3] ="   '*' Back         "
	for i in range(0,4):
		Message[i+4] = "                    "
